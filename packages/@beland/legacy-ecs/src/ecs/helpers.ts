...
export function openExternalURL(url: string) {
  if (typeof bld !== 'undefined') {
    bld.openExternalUrl(url)
  } else {
    error('ERROR: openExternalURL bld is undefined')
  }
}
...
export function openNFTDialog(scr: string, comment: string | null = null) {
  if (typeof bld !== 'undefined') {
    ...
    bld.openNFTDialog(matches[1], matches[2], comment)
  } else {
    error('ERROR: openNFTDialog bld is undefined')
  }
}
