...
export class RaycastEventSystem implements ISystem {
  activate(engine: Engine) {
    ...
    if (typeof bld !== 'undefined') {
      bld.subscribe('raycastResponse')
    }
  }

  deactivate() {
    if (typeof bld !== 'undefined') {
      bld.unsubscribe('raycastResponse')
    }
  }
}
...
export class PointerEventSystem implements ISystem {
  activate(engine: Engine) {
    ...
    if (typeof bld !== 'undefined') {
      bld.subscribe('pointerUp')
      bld.subscribe('pointerDown')
      bld.subscribe('actionButtonEvent')
    }
  }

  deactivate() {
    if (typeof bld !== 'undefined') {
      bld.unsubscribe('pointerUp')
      bld.unsubscribe('pointerDown')
      bld.unsubscribe('actionButtonEvent')
    }
  }
}
...
export class UUIDEventSystem implements ISystem {
  ...
  activate(engine: Engine) {
    ...
    if (typeof bld !== 'undefined') {
      bld.subscribe('uuidEvent')
    }
  }

  deactivate() {
    if (typeof bld !== 'undefined') {
      bld.unsubscribe('uuidEvent')
    }
  }
