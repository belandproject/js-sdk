...
let internalBld: BelandInterface | void

/**
 * @internal
 * This function generates a callback that is passed to the Observable
 * constructor to subscribe to the events of the BelandInterface
 */
function createSubscriber(eventName: keyof IEvents) {
  return () => {
    if (internalBld) {
      internalBld.subscribe(eventName)
    }
  }
}
...
/**
 * @internal
 * This function adds _one_ listener to the onEvent event of bld interface.
 * Leveraging a switch to route events to the Observable handlers.
 */
export function _initEventObservables(bld: BelandInterface) {
  // store internal reference to bld, it is going to be used to subscribe to the events
  internalBld = bld

  if (internalBld) {
    internalBld.onEvent((event) => {
      switch (event.type) {
        case 'onEnterScene': {
