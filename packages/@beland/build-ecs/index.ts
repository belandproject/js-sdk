  ...
  try {
    const t = require.resolve(packageName, {
      paths: [
        cwd,
        cwd + '/node_modules',
        cwd + '/node_modules/beland-ecs/',
        cwd + '/node_modules/beland-ecs/node_modules'
      ]
    })
    if (t) return t
  } catch {}
...
async function emitFile(
  ...
  function loadLibOrJs(lib: string) {
    if (PRODUCTION) {
      ...
      return (
        loadJsLib(lib.replace(/\.js$/, '.min.js')) || loadJsLib(lib) || false
      )
    } else {
      return loadJsLib(lib) || false
    }
  }
  ...
  for (const [, file] of out) {
    if (
      ...
      // emit lib file if it is a beland lib
      ...
      if (PRODUCTION || cfg.isBelandLib) {
        // minify && source map
        const minifiedFile = getOutFile(
          cfg.isBelandLib ? file.path.replace(/\.js$/, '.min.js') : file.path
        )
...
function getConfiguration(
  ...
  let isBelandLib = false

  if (tsconfig.options.module !== ts.ModuleKind.AMD) {
    console.error('! Error: tsconfig.json: Beland only allows AMD modules')
    hasError = true
  }
  ...
  const libs: BelandLib[] = []
  const bundledLibs: string[] = []

  if (packageJson) {
    if (packageJson.belandLibrary) {
      isBelandLib = true
    } else {
      isBelandLib = false
  ...
  if (isBelandLib && sceneJson) {
    console.error(
      '! Error: project of type belandLibrary must not have scene.json'
    )
    process.exit(1)
  }
  ...
  if (isBelandLib && !packageJson) {
    console.error(
      '! Error: project of type belandLibrary requires a package.json'
    )
    process.exit(1)
  }
  ...
  if (!isBelandLib && process.env.NO_DEFAULT_LIBS !== '1') {
    // most of the beland scenes require the following libraries.

    // FIRST UNSHIFT: ECS, order matters, don't change it
    libs.unshift({
      main:
        process.env.ECS_PATH ||
        findLibraryEntryPoint('beland-ecs', ts.sys.getCurrentDirectory()) ||
        'beland-ecs/dist/index.js'
    })
    ...
    libs.unshift({
      main:
        ...
        findLibraryEntryPoint('@beland/amd', ts.sys.getCurrentDirectory()) ||
        'beland-ecs/artifacts/amd.js'
    })
  }
  ...
  bundledLibs.forEach((libName) => {
    ...
    if (resolved) {
      try {
        ...
        const belandLibrary = libPackageJson.belandLibrary

        let main: string | null = null
        let typings: string | null = null

        if (!belandLibrary) {
          throw new Error(`field "belandLibrary" is missing in package.json`)
        }
        ...
        if (!libPackageJson.main) {
          ...
          main = resolve(
            dirname(resolved),
            belandLibrary.main || libPackageJson.main
          )
        ...
        if (!libPackageJson.typings) {
          ...
          typings = resolve(
            dirname(resolved),
            belandLibrary.typings || libPackageJson.typings
          )
  ...
  if (libs.length && isBelandLib) {
    console.log(
      '! Error: this project of type belandLibrary includes bundledDependencies. bundledDependencies are only allowed in scenes.'
    )
    process.exit(1)
  }
  ...
  return Object.assign(tsconfig, { libs, isBelandLib })
}
