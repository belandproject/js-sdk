...
namespace loader {
  ...
  export function require(
    ...
    if (typeof dependencies === 'string') {
      if (registeredModules[dependencies]) {
        if (registeredModules[dependencies].bldamd === MODULE_LOADING) {
          throw new Error(
  ...
  function load(
    ...
    if (registeredModules[moduleName]) {
      ...
      if (registeredModules[moduleName].bldamd === MODULE_LOADING) {
    ...
    if (moduleName.indexOf('@') === 0) {
      ...
      if (typeof bld.loadModule === 'function') {
        bld
    ...
    bld.onStart(() => {
      ...
      for (const i in registeredModules) {
        if (registeredModules[i]) {
          if (registeredModules[i].bldamd === MODULE_LOADING) {
