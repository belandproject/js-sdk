...
/// <reference path="../../packages/beland-ecs/node_modules/@beland/posix/index.d.ts" />
...
const globalObject: {
  bld: Pick<BelandInterface, 'loadModule' | 'callRpc' | 'onStart'>
...
export function mockEnvironment(modules: ModulesMock) {
  ...
  it('mocks the environment', () => {
    const amdModuleRequire = require.resolve('../../packages/@beland/amd')
    const content = readFileSync(amdModuleRequire).toString()
    ;(globalObject as any).define = null
    delete (globalObject as any).bld
    delete (globalObject as any).onerror

    expect(globalObject.bld).toBeUndefined()
    ...
    expect(globalObject.bld).not.toBeUndefined()
