...
const ecsLocation = resolve(__dirname, '../../packages/beland-ecs')
...
describe('build-ecs: simple scene compilation', () => {
  ...
  it('ensure it uses not minified versions in .lib', () => {
    ...
    expect(lib).toContain(resolve('packages/@beland/amd/dist/amd.js'))
    expect(lib).toContain(
      resolve('packages/beland-ecs/dist/src/index.js')
    )
  })
})
...
describe('build-ecs: simple scene compilation, production mode', () => {
  ...
  it('ensure it uses minified versions in .lib', () => {
    ...
    expect(lib).toContain(resolve('packages/@beland/amd/dist/amd.min.js'))
    expect(lib).toContain(
      resolve('packages/beland-ecs/dist/src/index.min.js')
    )
  })
})
